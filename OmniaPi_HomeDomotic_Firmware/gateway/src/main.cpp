...
// Debug: message type counters
volatile int msgHeartbeatAck = 0;
volatile int msgCommandAck = 0;
volatile int msgOther = 0;
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
    ...
    if (msgType == MSG_HEARTBEAT_ACK) msgHeartbeatAck++;
    else if (msgType == MSG_COMMAND_ACK) msgCommandAck++;
    else msgOther++;

    Serial.printf("[RX] Type=0x%02X Len=%d From=%02X:%02X:%02X:%02X:%02X:%02X\n",
                  msgType, len,
    ...
    if ((msgType == MSG_COMMAND_ACK || msgType == MSG_STATE) && nodeIdx >= 0) {
        Serial.printf("[DEBUG] COMMAND_ACK received! len=%d, nodeIdx=%d\n", len, nodeIdx);
        if (len >= 3) {
            // Format: [msgType][channel][state]
            uint8_t channel = data[1];
            uint8_t state = data[2];
            Serial.printf("[DEBUG] channel=%d, state=%d\n", channel, state);
...
void setupWebServer() {
    ...
    server.on("/api/status", HTTP_GET, [](AsyncWebServerRequest *req) {
        ...
        doc["msgHeartbeatAck"] = msgHeartbeatAck;
        doc["msgCommandAck"] = msgCommandAck;
        doc["msgOther"] = msgOther;
        // Add Gateway MAC (STA interface - used for ESP-NOW)
        uint8_t mac[6];
        WiFi.macAddress(mac);
        char macStr[18];
