/**
 * OmniaPi Node Mesh - Status LED Driver Implementation
 */
    ...
    while (s_running) {
        switch (s_current_pattern) {
            ...
            case STATUS_LED_COMMISSIONING:
                // Triple blink pattern: blink-blink-blink-pause
                LED_ON();
                vTaskDelay(pdMS_TO_TICKS(100));
                LED_OFF();
                vTaskDelay(pdMS_TO_TICKS(100));
...
esp_err_t status_led_init(void)
{
    ...
    BaseType_t xret = xTaskCreate(led_task, "status_led", 2048, NULL, 2, &s_led_task);
    if (xret != pdPASS) {
